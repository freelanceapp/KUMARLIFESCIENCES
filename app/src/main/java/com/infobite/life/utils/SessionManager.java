...
import com.infobite.life.ui.activity.LoginMainActivity;
...
public class SessionManager {
    ...
    public void checkLogin() {
        if (!this.isLoggedIn()) {
            Intent i = new Intent(ctx, HomeNavigationActivity.class);
    ...
    public void logoutUser(HomeNavigationActivity mainActivity) {
        editor.clear();
        editor.commit();
        ActivityCompat.finishAffinity(mainActivity);
        Intent i = new Intent(ctx.getApplicationContext(), LoginMainActivity.class);
        i.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
        i.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        ctx.startActivity(i);
    }
